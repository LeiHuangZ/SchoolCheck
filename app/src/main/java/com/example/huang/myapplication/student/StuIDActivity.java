package com.example.huang.myapplication.student;
 ...
 * @author huang
 */
public class StuIDActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSpUtils = new SpUtils(StuIDActivity.this);
        ...
        public void run() {
            ...
            while(isReading){
                if(myCardManager != null){
                    studentNum = myCardManager.readCardBID();
                    if(studentNum == null){
                        try {
                            Thread.sleep(200);
...
//                        isReading = false;
    ...
    private Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            switch(msg.what){
                case 0:
                    //存储访客临时卡号
                    mSpUtils.saveStuCard(MainActivity.count, studentNum);
                    String result = "学生证：" + studentNum;
                    mNext.setEnabled(true);
                    mStuCard.setText(result);
                    Util.initSoundPool(StuIDActivity.this);
                    Util.play(1, 0);
                    break;
            }
        }
    ...
    protected void onStart() {
        ...
        new Thread(new StuIDActivity.ThreadRun()).start();
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.next: isReading = false;
                if (myCardManager != null) {
                    myCardManager.close();
                    myCardManager = null;
                }
                startActivity(new Intent(StuIDActivity.this, EndActivity.class).addFlags(1));
                break;
            case R.id.finish:
                isReading = false;
                if (myCardManager != null) {
                ...
                Intent intent = new Intent(StuIDActivity.this, EndActivity.class);
                intent.addFlags(1);
                startActivity(intent);
                mStuCard.setText("");
                mNext.setEnabled(false);
                mSpUtils.saveStuCard(MainActivity.count, "");
                break;
            default:
                break;
        }
