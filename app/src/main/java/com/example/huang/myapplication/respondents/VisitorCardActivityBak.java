 ...
 * @author huang
 */
public class VisitorCardActivityBak extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSpUtils = new SpUtils(VisitorCardActivityBak.this);
            ...
            if (!mNfcAdapter.isEnabled()) {
                new android.app.AlertDialog.Builder(this)
                        ...
                        .setNegativeButton("取消", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Toast.makeText(VisitorCardActivityBak.this,"您的设备未开启NFC，将无法读取卡片信息",Toast.LENGTH_SHORT).show();
                            }
                        }).show();
            }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(intent.getAction()) || NfcAdapter.ACTION_TECH_DISCOVERED.equals(intent.getAction())) {
            ...
            String visitorNum = Tools.Bytes2HexString(tag.getId(), tag.getId().length);

            //存储访客临时卡号
            mSpUtils.saveVisitorCard(MainActivity.count, visitorNum);
            result = "临时卡号：" + visitorNum;
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.next:
                startActivity(new Intent(VisitorCardActivityBak.this, VisitorFaceTakeActivity.class).addFlags(0));
                break;
            case R.id.finish:
                mSpUtils.saveVisitorCard(MainActivity.count, "");
                Intent intent = new Intent(VisitorCardActivityBak.this, VisitorFaceTakeActivity.class);
                startActivity(intent);
