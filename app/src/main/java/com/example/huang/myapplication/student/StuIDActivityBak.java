 ...
 * @author huang
 */
public class StuIDActivityBak extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSpUtils = new SpUtils(StuIDActivityBak.this);
            ...
            if (!mNfcAdapter.isEnabled()) {
                new android.app.AlertDialog.Builder(this)
                        ...
                        .setNegativeButton("取消", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Toast.makeText(StuIDActivityBak.this,"您的设备未开启NFC，将无法读取卡片信息",Toast.LENGTH_SHORT).show();
                            }
                        }).show();
            }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(intent.getAction()) || NfcAdapter.ACTION_TECH_DISCOVERED.equals(intent.getAction())) {
            ...
            String stuId = Tools.Bytes2HexString(tag.getId(), tag.getId().length);

            //存储学生证号
            mSpUtils.saveStuCard(MainActivity.count, stuId);
            result = "学生证：" + stuId;
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.next:
                startActivity(new Intent(StuIDActivityBak.this, EndActivity.class).addFlags(1));
                break;
            case R.id.finish:
                Intent intent = new Intent(StuIDActivityBak.this, EndActivity.class);
                intent.addFlags(1);
                startActivity(intent);
                mStuCard.setText("");
